 ...
 * @param len   字节（1B） 长度
 */
int f0_flash_write(uint32_t address, uint16_t len, uint8_t *p_data)
{
    ...
    trace_debug("f0_flash_write erase done\n\r");
    ...
    for(i = 0; i < len; i++)
    {
        ...
        {
            trace_debug("f0_flash_write write success\n\r");
        }
        else
        {
            trace_debug("f0_flash_write write error\n\r");
        }
